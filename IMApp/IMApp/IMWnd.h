...
typedef enum {
    ...
    NODE_QUIT_GROUP   = 8,
    NODE_DELMEM_GROUP = 9,
    NODE_CREATE_FRIEND = 10,
    NODE_DELETE_FRIEND = 11,
    NODE_FRIEND_VIEW = 12,
}NodeViewType;
...
public:
    ...
    int MsgBox(std::string title, const char* fmt, ...);
    static int MsgBoxEx(std::string title, const char* fmt, ...);
    //void InitSDK();
    void OnUnInitSDKBtn();
    void OnLoginBtn();
    void OnLogoutBtn();
    ...
    std::string login_id;
    std::string path_;
    void AddFriend(FriendInfo& fr);
    void DelFriend(std::string id);
...
public:
  ...
  //  CControlUI* m_LoginView = nullptr;
    ...
    CTreeNodeUI* m_FriendNode = nullptr;
