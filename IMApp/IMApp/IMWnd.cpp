...
CIMWnd::CIMWnd()
{
	...
	char strLoginId[MAX_PATH] = { 0 };
	m_SkilFile = "frame.xml";

	GenerateTestUserSig* instance = GenerateTestUserSig::GetInstance();
	instance->GetLogPath(strLogPath);
	path_ = strLogPath;
	instance->GetLoginId(strLoginId);
	login_id = strLoginId;
}
...
void CIMWnd::Init()
{
	...
	TIMSetFriendAddRequestCallback([](const char* json_friend_add_request_pendency_array, const void* user_data)
	{
		...
		Json::Value root;
		string userid;
		if (reader.parse(json_friend_add_request_pendency_array, root))  // reader将Json字符串解析到root，root将包含Json里所有子元素  
		{
			userid = root[0]["friend_add_pendency_identifier"].asString();  // 访问节点，upload_id = "UP000000"  
			
		}
		//判断是否同意
		int dwAddValue = ths->MsgBox("AddRequest", "Id:%s", userid.c_str());
		
		//处理好友请求
		Json::Value json_handle_friend_add_param;
		json_handle_friend_add_param[kTIMFriendResponeIdentifier] = userid;
		if (dwAddValue ==1) //表示接收
		{
		...
		{
			//==2 表示接收
		   json_handle_friend_add_param[kTIMFriendResponeAction] = ResponseActionReject;
		}
		...
		int ret = TIMFriendshipHandleFriendAddRequest(json_handle_friend_add_param.toStyledString().c_str(), [](int32_t code, const char* desc, const char* json_params, const void* user_data) 
		{
			...
			if (ERR_SUCC != code)
			{
				ths->MsgBox("AddRequest", "Failure!code:%d desc", code, desc);
				// 
				return;
			}
			else
			{
				...
				Json::Value root;
				string userid;
				if (reader.parse(json_params, root))  // reader将Json字符串解析到root，root将包含Json里所有子元素  
				{
					userid = root["friend_result_identifier"].asString();  // 访问节点，upload_id = "UP000000"  
...
void CIMWnd::Notify(TNotifyUI& msg)
{
	...
	{
		if (msg.pSender->GetName() == _T("conv_node"))
		{ //右键菜单 新建会话
			m_CurMenuNode = msg.pSender;
			OnPopupMenu(MENU_CREATE_CONV);
		}
		if (msg.pSender->GetName() == _T("group_node"))
		{ //右键菜单 新建群组或加入群组
			m_CurMenuNode = msg.pSender;
		...
		if (msg.pSender->GetName() == _T("group_node_item"))
		{  //右键菜单 删除群组 邀请加入 退出群组
			m_CurMenuNode = msg.pSender;
			OnPopupMenu(MENU_DELETE_GROUP);
		}
		if (msg.pSender->GetName() == _T("groupmem_node_item"))
		{  //右键菜单 删除群成员
			m_CurMenuNode = msg.pSender;
	...
	if (msg.sType == DUI_MSGTYPE_CLICK)
	{
		...
		if (msg.pSender->GetName() == _T("initsdk_btn"))
		{ //初始化SDK
		  //  OnInitSDKBtn();
		}
		...
		if (msg.pSender->GetName() == _T("create_friend_btn"))
		{
			OnCreateFriendBtn();
		}
		if (msg.pSender->GetName() == _T("delete_friend_btn"))
		{
			OnDelFriendBtn();
		}
		...
		if (msg.pSender->GetName() == _T("sendface_bth"))
		{ //选择表情
			OnSendFaceBtn();
		}
		if (msg.pSender->GetName() == _T("sendimage_btn"))
		{ //选择上传图片
			OnSendImageBtn();
		}
		if (msg.pSender->GetName() == _T("sendfile_btn"))
		{ //选择上传文件
			OnSendFileBtn();
		}
		if (msg.pSender->GetName() == _T("sendmsg_btn"))
		{ //消息发送
			OnSendMsgBtn();
		}
	...
	if (msg.sType == DUI_MSGTYPE_ITEMCLICK)
	{
		...
		if (msg.pSender->GetName() == _T("create_friend_node"))
		{
			m_CurMenuNode = nullptr;
			ChangeNodeView(NODE_CREATE_FRIEND);
		}
		if (msg.pSender->GetName() == _T("delete_friend_node"))
		{
			m_CurMenuNode = nullptr;
			ChangeNodeView(NODE_DELETE_FRIEND);
		}
		//点击显示会话界面
		if ((msg.pSender->GetName() == _T("conv_node_item")) ||
			(msg.pSender->GetName() == _T("group_node_item"))||
			(msg.pSender->GetName() == _T("friend_node_item")))
		{
	...
	if (msg.sType == DUI_MSGTYPE_ITEMSELECT)
	{
		...
		if (msg.pSender->GetName() == _T("delmem_groupid_combo"))
		{  //更新删除是的成员列表
...
void CIMWnd::InitWindow()
{
	...
	m_FriendNode = static_cast<CTreeNodeUI*>(m_pm.FindControl(_T("friend_node")));
 ...
 //   CListLabelElementUI* ele = new CListLabelElementUI;
  ...
  //  m_SdkAppIdCombo->SelectItem(0);
   ...
   /* m_UserIdEdit = static_cast<CEditUI*>(m_pm.FindControl(_T("userid_edit")));
	...
	SetControlVisible(_T("logout_btn"), true);
	SetControlVisible(_T("test_btn"), true);

	SetControlVisible(_T("cur_login_info"), true);
	//GenerateTestUserSig* instance = GenerateTestUserSig::GetInstance();
	//instance->GetLoginId(strLoginid);
	SetControlText(_T("cur_login_info"), UTF82Wide(login_id).c_str());
	InitConvList();
	GetGroupJoinedList();
	SetControlText(_T("cur_loginid_lbl"), UTF82Wide(login_id).c_str());
	InitFriendList();
}
int CIMWnd::MsgBox(std::string title, const char* fmt, ...)
{
	std::string tmp;
	va_list ap;
	va_start(ap, fmt);
	...
	CMsgBox msgbox;
return	msgbox.DuiMessageBox(this, UTF82Wide(tmp).c_str(), UTF82Wide(title).c_str(),4,0);
}

int CIMWnd::MsgBoxEx(std::string title, const char* fmt, ...)
{
	std::string tmp;
	va_list ap;
	va_start(ap, fmt);
	...
	CMsgBox msgbox;
return	msgbox.DuiMessageBox(nullptr, UTF82Wide(tmp).c_str(), UTF82Wide(title).c_str());
}
...
//    std::string path = path_;// Wide2UTF8(m_LogPath->GetText().GetData());
...
void CIMWnd::InitFriendList()
{ // 获取会话列表
	int ret = TIMFriendshipGetFriendProfileList([](int32_t code, const char* desc, const char* json_param, const void* user_data) 
	{
		CIMWnd* ths = (CIMWnd*)user_data;
		...
		{
			CIMWnd::GetInst().Logf("GetFriendList", kTIMLog_Error, "Json Parse Conv List Failure! error:%s", reader.getFormattedErrorMessages().c_str());
			return;
		}
		CIMWnd::GetInst().Logf("FriendList", kTIMLog_Info, json_param);
		for (Json::ArrayIndex i = 0; i < json_conv_list.size(); i++)
		{
			FriendInfo friendinfo;
			Json::Value& fr = json_conv_list[i];

			friendinfo.id = fr[kTIMFriendProfileIdentifier].asString();
			
			CIMWnd::GetInst().AddFriend(friendinfo);
		}
	}, this);
	Logf("Init", kTIMLog_Info, "TIMFriendshipGetFriendProfileList ret %d", ret);

}

void CIMWnd::InitConvList()
{ // 获取会话列表
...
void CIMWnd::DelConv(std::string id, uint32_t type)
{
	if (type == TIMConvType::kTIMConv_C2C)
	{
		...
		{
			if (CIMWnd::GetInst().convs[i].id == id)
			{ // 删除
	...
	else if (type == TIMConvType::kTIMConv_Group)
	{
		...
		{
			if (CIMWnd::GetInst().groups[i].id == id)
			{ // 删除
...
void CIMWnd::DownloadMessageElem(uint32_t flag,
	uint32_t type, std::string id,
	uint32_t business_id, std::string url,
	std::string name)
{
...
std::string CIMWnd::GBToUTF8(const char* str)
{
	std::string result;
	WCHAR* strSrc;
	CHAR *szRes;
...
void CIMWnd::OnCreateFriendBtn()
{  //添加好友 按钮
	CDuiString tmp = GetControlText(_T("friend_userid_edit"));
	std::string userid = Wide2UTF8(tmp.GetData());
	Json::Value json_add_friend_param;
	//请求加好友对应的 UserID
	json_add_friend_param[kTIMFriendshipAddFriendParamIdentifier] = userid.c_str();
	//请求添加好友的好友类型
	json_add_friend_param[kTIMFriendshipAddFriendParamFriendType] = FriendTypeBoth;
	//kTIMFriendshipAddFriendParamAddSource
	json_add_friend_param[kTIMFriendshipAddFriendParamAddSource] = login_id.c_str();
	...
	int ret = TIMFriendshipAddFriend(json_add_friend_param.toStyledString().c_str(), [](int32_t code, const char* desc, const char* json_params, const void* user_data) 
	{
		...
		if (ERR_SUCC != code)
		{
			...
			ths->MsgBox("Add Friend", "Failure!code:%d desc", code, desc);
				return;
			
		}
		//else
		//{
		//	ths->MsgBox("Add Friend", ths->GBToUTF8("消息已经发送").c_str());
		//	return;
		//}
		 }, this);
	UpdateNodeView();
...
void CIMWnd::OnPopupMenu(MenuType mt)
{ //右键弹出菜单
	CDuiPoint point(0, 0);
	GetCursorPos(&point);
	CDuiString xml;
...
bool CIMWnd::SendMsg(const char* json_msg)
{
	...
	{
		FriendInfo& fr = m_friends[i];
		if (tag != (UINT_PTR)&fr)
		{
			continue;
		}
		json_value_msg[kTIMMsgConvId] = fr.id;
		json_value_msg[kTIMMsgConvType] = kTIMConv_C2C;
		Json::Value json_value_msgs;
		json_value_msgs.append(json_value_msg);
		msgs.push_back(json_value_msgs.toStyledString());
		return TIMMsgSendNewMsg(fr.id.c_str(), kTIMConv_C2C, json_value_msg.toStyledString().c_str(), commcb, this) == TIM_SUCC;
	}
...
void CIMWnd::ShowMsgs(std::vector<std::string>& msgs, std::string conv_id, TIMConvType conv_type)
{
	for (std::size_t i = 0; i < msgs.size(); i++)
	{
		...
		for (Json::ArrayIndex i = json_value_msgs.size(); i > 0; i--)
		{  // 遍历Message
			...
			for (Json::ArrayIndex i = 0; i < json_value_elems.size(); i++)
			{ //遍历 elem array
...
void CIMWnd::ParseMsg(const char* json_msg_array)
{  //解析消息找到对应的会话信息
	...
	for (Json::ArrayIndex i = 0; i < json_value_msgs.size(); i++)
	{  // 遍历Message
...
void CIMWnd::ChangeMainView(MainViewType before, MainViewType after)
{ //修改主界面
	RECT rect;
...
void CIMWnd::ChangeNodeView(NodeViewType vt)
{ //修改view，更新各个控件的状态
...
void CIMWnd::UpdateGroupCombo(TCHAR* combo_name)
{ // 更新群组ID combo
...
void CIMWnd::AddFriend(FriendInfo& fr)
{
	m_friends.push_back(fr);
}

void CIMWnd::DelFriend(std::string id)
{
}
...
void CIMWnd::UpdateNodeView()
{
	...
	if (nullptr != m_CurMenuNode)
	{      // 当前右键点击的Node
		Menu_Node_Tag = m_CurMenuNode->GetTag();
		m_CurMenuNode = nullptr;
	}
	if (nullptr != m_CurShowConvNode)
	{  // 当前显示的会话的Node
	...
	{  // 显示会话Node列表
		...
		m_ConvNode->Add(node);
		if (Menu_Node_Tag == (UINT_PTR)&conv)
		{  //更新当前Node
			m_CurMenuNode = node;
		}
	...
	for (uint32_t i = 0; i < m_friends.size(); i++)
	{  // 显示好友Node列表
		FriendInfo& fr = m_friends[i];
		CTreeNodeUI* node = new CTreeNodeUI(m_FriendNode);
		if (node == nullptr)
		{
			continue;
		}
		node->SetName(_T("friend_node_item"));
		node->SetContextMenuUsed(true);
		node->SetTag((UINT_PTR)&fr); //保存会话信息
		node->SetItemText(UTF82Wide(fr.id).c_str());
		m_FriendNode->Add(node);
		if (Menu_Node_Tag == (UINT_PTR)&fr)
		{  //更新当前Node
			m_CurMenuNode = node;
		}
		if (ShowConv_Node_Tag == (UINT_PTR)&fr)
		{
			m_CurShowConvNode = node;
		}
	}
	m_FriendNode->Invalidate();
	...
	{ // 显示群组Node列表
		...
		if (Menu_Node_Tag == (UINT_PTR)&group)
		{ //更新会话节点
			m_CurMenuNode = node;
		}
	...
	{  //显示和更新会话消息
		...
		if (m_CurShowConvNode && m_CurShowConvNode->GetTag())
		{
			...
			{  // 显示好友消息
				...
				m_ConvGroupInfoView->SetVisible(false);
				m_ConvUserInfoView->SetVisible(true);
				SetControlText(_T("conv_userinfo_lab"), UTF82Wide(fr.id).c_str());  //显示会话名称
				ShowMsgs(msgs, fr.id, kTIMConv_C2C);
			}
		}
	}

	if (node_type_ == NODE_DELETE_CONV)
	{   //更新删除会话界面
